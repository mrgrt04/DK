...
def report(
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        if quality_flags["has_constant_columns"]:
            f.write(f"  - {quality_flags['constant_columns']}\n")
        ...
        if quality_flags["has_high_cardinality_categoricals"]:
            f.write(f"  - {quality_flags['high_cardinality_categoricals']}\n")
        f.write("\n")
        ...
        if missing_df.empty:
            ...
            # Проблемные колонки по порогу
            problematic = missing_df[missing_df["missing_share"] >= float(min_missing_share)]

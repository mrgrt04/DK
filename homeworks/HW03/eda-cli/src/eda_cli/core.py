...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # 1) Константные колонки: все значения одинаковые (включая NaN)
    ...
    # 2) Высокая кардинальность категориальных: много уникальных значений
    # Порог: минимум 50 уникальных и доля уникальных > 0.5 от числа строк.
    high_card_cols: List[str] = []
    if summary.n_rows > 0:
        for c in summary.columns:
            if c.is_numeric:
                continue
            if c.unique >= 50 and (c.unique / summary.n_rows) > 0.5:
                high_card_cols.append(c.name)
